...
[기능 명세에서 추출: 주요 요구사항 + 연구에서 도출한 기술적 접근법]

## 기술 컨텍스트

<!--
  조치 필요: 이 섹션의 내용을 프로젝트의 기술 세부사항으로 교체합니다.
  여기 제시된 구조는 반복 프로세스를 안내하기 위한 권고 사항입니다.
-->
...
*게이트: Phase 0 연구 전에 통과해야 합니다. Phase 1 설계 후 재검사합니다.*
...
### 소스 코드 (저장소 루트)
<!--
  조치 필요: 아래 플레이스홀더 트리를 이 기능의 구체적인 레이아웃으로 교체합니다.
  사용하지 않는 옵션은 삭제하고 선택한 구조를 실제 경로로 확장합니다
  (예: apps/admin, packages/something). 제공되는 계획에는 옵션 레이블이
  포함되지 않아야 합니다.
-->
...
**구조 결정**: [선택한 구조를 문서화하고 위에 캡처된 실제 디렉터리를 참조]

## 복잡성 추적

> **헌장 검사에 정당화가 필요한 위반이 있는 경우에만 채우세요**

| 위반 | 필요한 이유 | 더 간단한 대안이 거부된 이유 |
|-----------|------------|-------------------------------------|
| [예: 4번째 프로젝트] | [현재 필요] | [3개 프로젝트로 불충분한 이유] |
| [예: 저장소 패턴] | [특정 문제] | [직접 DB 접근이 불충분한 이유] |
